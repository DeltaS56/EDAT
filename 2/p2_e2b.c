...
int compare_artist(const void *m1, const void *m2) {
  return strcmp(music_getArtist((Music *)m1), music_getArtist((Music *)m2));
}

Status mergeStacks(Stack *sin1, Stack *sin2, Stack *sout, P_ele_cmp cmp_func) {
  void *e = NULL;
  Stack *ps = NULL;
  ...
  while (stack_isEmpty(sin1) == FALSE && stack_isEmpty(sin2) == FALSE) {
    if (cmp_func(stack_top(sin1), stack_top(sin2)) > 0) {
      e = stack_pop(sin1);
    } else {
      e = stack_pop(sin2);
    }
...
int main(int argc, char *argv[]) {
  ...
  if (mergeStacks(s1, s2, s3, compare_artist) == ERROR) {
    radio_free(r1);
    radio_free(r2);
    stack_free(s1);
    stack_free(s2);
